...
class Draxira_Products
{
    ...
    private function mc_drax_load_product_data()
    {
        ...
        $handle = fopen($this->mc_drax_csv_file_path, 'r');
        if ($handle === false) {
            $this->mc_drax_product_data = [];
            $this->mc_drax_variation_data = [];
            return;
    ...
    private function mc_drax_get_product_meta_keys()
    {
        ...
        $excluded_keys = [
            ...
            '_stock_status', '_manage_stock', '_stock',
            '_weight', '_length', '_width', '_height',
            '_virtual', '_downloadable', '_tax_status',
            '_tax_class', '_purchase_note', '_featured',
            '_visibility', '_backorders', '_sold_individually',
            '_product_image_gallery', '_draxira_dummy_content'
        ];
        ...
        while ($created < $count && $attempts < $max_attempts) {
            ...
            if ($faker) {
                if ($taxonomy === 'product_cat') {
                    ...
                    $term_name = ucwords($faker->words(rand(1, 3), true));
                }
            } else {
    ...
    public function mc_drax_ajax_get_product_meta()
    {
        ...
        <div class="product-meta-section">
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        ...
                        <span class="description"><?php esc_html_e('Select who will be the author of generated products', 'draxira'); ?></span>
                     </div>
                  </div>
                 </div>
                <tr>
            ...
            <?php if (!empty($taxonomies)): ?>
                ...
                <p class="description"><?php esc_html_e('When "Create Terms" is enabled, dummy terms will be generated using Faker for custom taxonomies.', 'draxira'); ?></p>
                <table class="widefat">
                    <thead>
                        <tr>
            ...
            <?php if (!empty($meta_keys)): ?>
                ...
                <p class="description"><?php esc_html_e('Configure how custom fields (ACF, CMB2, custom meta) should be filled using Faker.', 'draxira'); ?></p>
                <table class="widefat">
                    <thead>
                        <tr>
            ...
            <tbody>
                <?php foreach ($dummy_products as $product):
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="<?php echo esc_url(get_edit_post_link($product->ID)); ?>" class="button button-small"><?php esc_html_e('Edit', 'draxira'); ?></a>
        ...
        <div class="wrap draxira">
            ...
            <?php if ($data_count === 0): ?>
                <div class="notice notice-warning">
                    <p><strong><?php esc_html_e('Warning:', 'draxira'); ?></strong> <?php printf(esc_html__('No product data found in %s. Please ensure the CSV file exists and contains valid product data.', 'draxira'), '<code>woo-data/woo.csv</code>'); ?></p>
                </div>
            <?php else: ?>
                <div class="notice notice-info">
                    <p><strong><?php echo esc_html($data_count); ?></strong> <?php esc_html_e('products loaded from CSV file. Products will be generated using this data.', 'draxira'); ?></p>
                </div>
            <?php endif; ?>
            ...
            if ($results) {
                ...
                echo '<div class="notice ' . $notice_class . '"><p>' . esc_html($results['message']) . '</p></div>';
            }
            ?>

            <h2 class="nav-tab-wrapper">
                <a href="#generate-products-tab" class="nav-tab nav-tab-active"><?php esc_html_e('Generate Products', 'draxira'); ?></a>
            ...
            <div id="generate-products-tab" class="tab-content active">
                <form method="post" action="" id="generate-products-form">
                    ...
                    <p class="submit">
                        <input type="submit" name="generate_products" class="button button-primary" value="<?php esc_attr_e('Generate Products', 'draxira'); ?>">
                    </p>
                </form>
            </div>
            ...
            <div id="manage-products-tab" class="tab-content">
                    ...
                    <form method="get" action="" class="filter-dummy-products">
                        ...
                        <table class="form-table">
                            <tr>
                                ...
                                <td>
                                    <button type="button" id="load-dummy-products" class="button"><?php esc_html_e('Load Dummy Products', 'draxira'); ?></button>
                                </td>
                            </tr>
                        </table>
                    </form>
                ...
                <div id="delete-section"
                    ...
                    <p class="description"><strong><?php esc_html_e('Warning:', 'draxira'); ?></strong> <?php esc_html_e('This will permanently delete ALL dummy products (including variations, meta, terms, images, etc.). This cannot be undone.', 'draxira'); ?></p>
